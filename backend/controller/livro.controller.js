...
router.post('/cadastrar', async (req, res) => {
  try {
    const { isbn, titulo, id_autor, id_editora, qtd_disponivel, edicao, id_categoria, id_subcategoria, url_capa } = req.body;

    if (!isbn || !titulo || !id_autor || !id_editora || !qtd_disponivel || !id_categoria || !id_subcategoria || !url_capa) {
      return res.status(400).json({ erro: 'Campos obrigatórios ausentes.' });
    }

    const livro = await livroDAO.cadastrarLivro(isbn, titulo, id_autor, id_editora, qtd_disponivel, edicao, id_categoria, id_subcategoria, url_capa);
...
router.put('/:id', async (req, res) => {
  ...
  if (!isbn || !titulo || !id_autor || !id_editora || !qtd_disponivel || !id_categoria || !id_subcategoria || !url_capa || isNaN(id_livro)) {
    return res.status(400).json({ erro: 'Dados inválidos.' });
  }

  try {
    const livroAtualizado = await livroDAO.atualizarLivro(id_livro, isbn, titulo, id_autor, id_editora, qtd_disponivel, edicao, id_categoria, id_subcategoria, url_capa);
    if (!livroAtualizado) {
